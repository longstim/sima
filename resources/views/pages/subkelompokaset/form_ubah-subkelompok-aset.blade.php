...
@section('page_heading','Edit Sub Kelompok Aset')
@section('breadcrumb')
...
<div class="row">
	<div class="col-md-12">
		...
		<div class="card card-outline card-primary shadow">
			<form role="form" id="ubahsubkelompokaset" method="post" action="{{url('subkelompokaset/proses-ubah-subkelompok-aset')}}" >
		  	{{ csrf_field() }}
			...
			<div class="card-body">
				...
				<div class="card">
					...
					<div class="card-body">
						<input type="hidden" name="id" value="{{$subkelompokaset->id}}">
					        ...
					        <label>Kelompok Aset<a style="color:red;">&#42;</a></label>
					         <select name="kelompokaset" id="id_kelompok_aset" class="form-control select2bs4" onchange="datakelompokaset(this.value)" style="width: 100%;" required>
		                        ...
		                        @foreach($kelompokaset as $data)
		                            <option value="{{$data->id}}" @if($data->id == $subkelompokaset->id_kelompok_aset) selected @endif>{{"[".$data->kode."] ".$data->nama}}</option>
		                        @endforeach
		                    </select>
					    </div>
					    <div class="form-group col-md-12">
					      	<div class="row">
								...
								<div class="col-md-4">
								    ...
								    <input type="text" name="kode" class="form-control{{ $errors->has('kode') ? ' is-invalid' : '' }}" id="txtKode" value="{{$subkelompokaset->kode}}" placeholder="Kode" required>
							         @if ($errors->has('kode'))
				      	...
				      	<div class="form-group col-md-12">
				        	...
				        	<input type="text" name="nama" class="form-control" id="txtNama" value="{{$subkelompokaset->nama}}" placeholder="Nama" required>
				      	</div>
				      	<div class="form-group col-md-12">
				        	<label>Satuan<a style="color:red;">&#42;</a></label>
				        	<input type="text" name="satuan" class="form-control" id="txtSatuan" value="{{$subkelompokaset->satuan}}" placeholder="Satuan" required>
				      	</div>
				      	<div class="form-group col-md-12">
				        	<label>Keterangan</label>
				        	<textarea name="keterangan" id="txtKeterangan" class="form-control" placeholder="Keterangan" rows="2">{{$subkelompokaset->keterangan}}</textarea>
				      	</div>
			     	</div>	
			 	</div>
			 	<!-- Card -->
